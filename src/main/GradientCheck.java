...
public class GradientCheck {
	public static void main(String[] args) throws Exception{
		...
		for(int i = 0; i<theta.row_num; i++){
			...
			double f_theta_h = new CrossEntropyCost().cost(nn.predict(data), label);
			nn.setParams(theta_minus_h);
			double f_theta_minus_h = new CrossEntropyCost().cost(nn.predict(data), label);
